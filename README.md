...
- `DOTA_CHANNEL_ID`, `DOTA_CRON`, `DOTA_TIMEZONE` — канал/время/таймзона ежедневной статистики Dota (по умолчанию 23:00, канал погоды)
- `DOTA_ALERT_USER_ID`, `DOTA_ALERT_ACCOUNT_ID`, `DOTA_ALERT_MIN_HOURS`, `DOTA_ALERT_CRON`, `DOTA_ALERT_CHANNEL_ID` — напоминание «пора играть» (см. раздел Dota 2)
...
**Напоминание «пора играть».** Каждый день в `DOTA_ALERT_CRON` (по умолчанию 13:00) бот считает, сколько сыграно сегодня по `DOTA_ALERT_ACCOUNT_ID`. Если меньше `DOTA_ALERT_MIN_HOURS` часов — пингует `DOTA_ALERT_USER_ID` (это Discord ID) текстом «СЕГОДНЯ ДЛИТЕЛЬНОСТЬ ИГРЫ В ДОТУ : … критически низкое значение…». Если у OpenDota нет данных по аккаунту (скрытый профиль), напоминание пропускается, чтобы не пинговать зря.

Временная команда для проверки: `/dotatest` (параметры `account_id`, `min_hours`, `user`, `force` — отправить напоминание даже без нарушения порога). Чтобы убрать — удалить файл `src/commands/dotatest.js`.
