...
- `MINECRAFT_API_TOKEN` — токен API заявок (`BOT_API_TOKEN` в `.env` minecraft-server), секретный
- `MINECRAFT_APPROVER_IDS` — Discord ID через запятую: кому в личку приходят заявки на игру (см. раздел «Minecraft-сервер»)
- `MINECRAFT_APPLICATIONS_CRON` — как часто забирать новые заявки (по умолчанию каждые 30 секунд)
...
- `/minecraft apply nick:<ник> comment:<необязательно>` — заявка на игру (на сервере whitelist). Заявку можно подать и на сайте (`https://<домен>/apply`)
- Новые заявки (каждые `MINECRAFT_APPLICATIONS_CRON`, из Discord — сразу) приходят в личку всем из `MINECRAFT_APPROVER_IDS` с кнопками «Одобрить»/«Отклонить». Решает тот, кто нажал первым, у остальных сообщение обновляется. Одобрение добавляет ник в whitelist (link-server выполняет через RCON `simplewhitelist add <ник>`), подавшему из Discord бот пишет о решении в личку, подавший с сайта видит статус на странице заявки. Какие сообщения к какой заявке отправлены — `data/minecraft-applications.json`
